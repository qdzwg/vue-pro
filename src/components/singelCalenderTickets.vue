          ...
          <tbody>
            <tr v-for="(week,index) in daysArr" :key="index">
              <td
                ...
                :key="dayIndex"
                @click="getDate(day)"
                :class="{'active':(selectDate==day.date),'not-after-today': !day.isAfterToday||(day.needCheck && (day.salePrice !== 0 && !day.salePrice))}"
              >
...
export default {
  ...
  methods: {
    ...
    getDate(day) {
      let libsDay = [];
      if ( !day.needCheck || (day.isAfterToday && (day.salePrice || day.salePrice == 0))) {
    ...
    initDate(year, month) {
      ...
      if (this.url&&this.$route.query.merchantFamilyTicketId||this.$route.query.merchantFamilyInfoId) {
        this.postAxios(
          this.url?this.url:"/merchant/api/merchantFamilyInfo/priceCalendarList",
          this.url?{
          ...
          for (var j = 1; j <= days; j++) {
            ...
            let day = {
              ...
              date: date, //年-月-日
              salePriceStatus: (this.url?daysData[j - 1].salePrice:daysData[j - 1].priceShow)?"T":"F", //在售状态
              day: date == this.currentdate ? '今天' : j,
              salePrice: this.url?daysData[j - 1].salePrice:daysData[j - 1].priceShow, //售价
