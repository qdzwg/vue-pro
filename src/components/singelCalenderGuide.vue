          ...
          <tbody>
            <tr v-for="(week,index) in daysArr" :key="index">
              <td
                ...
                :key="dayIndex"
                @click="getDate(day)"
                :class="{'active':(selectDate==day.date),'not-after-today': !day.isAfterToday||(day.needCheck && (day.salePrice !== 0 && !day.salePrice)),'gray': !day.needCheck || !(day.isAfterToday && (day.salePrice || day.salePrice == 0) && (day.stockModel=='unlimit'?true : day.stock>0))}"
              >
                <template v-if="day.currentMonth">
                  ...
                  <div class="price">
                    <!-- 价格展示 -->
                    <!-- <template
                      v-if="showPrice && day.isAfterToday && typeof day.salePrice !== 'undefined'"
                    >￥{{day.salePrice}}</template> -->
                    <!-- 库存展示 -->
                    <template
                      v-if="day.isAfterToday && typeof day.stockModel !== 'undefined'"
                    >余{{day.stockModel=='unlimit'? '999+' : day.stock}}</template>
                  </div>
                </template>
              </td>
            </tr>
...
export default {
  ...
  methods: {
    ...
    getDate(day) {
      let libsDay = [];
      if ( !day.needCheck || (day.isAfterToday && (day.salePrice || day.salePrice == 0) && (day.stockModel=='unlimit'?true : day.stock>0))) {
    ...
    initDate(year, month) {
      ...
      if (this.url) { 
        this.postAxios(this.url,Object.assign(this.httpOption,{year: year,month: month + 1})
        ).then(res => {
          if (res.status != 200) {
            return;
          }
          ...
          for (var j = 1; j <= days; j++) {
            ...
            let __date = daysData.find((_date) => {
            ...
            let day = {
              ...
              date: date, //年-月-日
              salePriceStatus: __date.salePrice?"T":"F", //在售状态
              day: date == this.currentdate ? '今天' : j,
              salePrice: __date.salePrice, //售价
              stock: __date.stock || 0, //库存
              stockModel: __date.stockModel || 'day', //库存模式
              isAfterToday:
          ...
          this.daysFullData = daysCanlender
          console.log(daysCanlender)
          if ((month + 1) === 12) {
            this.getNextMouth((year +1),1)
          }else {
            this.getNextMouth(year,(month + 1))
          }
