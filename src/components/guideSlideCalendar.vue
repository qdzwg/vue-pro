<template>
  <div class="slide-calendar tickets-slide-calendar">
      ...
      <div class="slide-calendar-list tickets-slide-calendar-list">
        <swiper :options="swiperOption" ref="mySwiper" @click.prevent="selectedDate">
          ...
          <swiper-slide
            ...
            :key="index"
            :class="{
              'active':swiperActiveIndex === index && ((item.salePrice || item.salePrice == 0 ) && (item.stockModel=='unlimit'?true : item.stock>0)),
              'gray':!(typeof item.salePrice !== 'undefined' && (item.stockModel=='unlimit'?true : item.stock>0))}"
          > 
            <div class="swiper-slide-time">{{item.week+item.date}}</div>
            <div v-if="showPrice && (item.salePrice ||item.salePrice == 0) && (item.stockModel=='unlimit'?true : item.stock>0)" class="swiper-slide-price">余{{item.stockModel=='unlimit'? '999+' : item.stock}}</div>
            <div v-else-if="showPrice && (item.salePrice ||item.salePrice == 0) && item.stockModel=='day'" class="swiper-slide-price">已售完</div>
    ...
    <singel-calender ref="calendar" @initDate="initDate" :showPrice="showPrice" :url="httpUrl" :httpOption="httpParams" @getDate="getSingeDate" :singeDate="curDate"></singel-calender>
  </div>
</template>
<script>
...
import singelCalender from "@/components/singelCalenderGuide";
export default {
  components: { 
    swiper,
    swiperSlide,
  ...
  created() {
    this.getCurDate();
    this.getDaysInfo(5, new Date(this.slideDate));
  },
  mounted() {
  ...
  methods: {
    initDate(val){
      ...
      this.getDaysInfo(5, initDate, val);
    },
    // 打开日历弹框
    moreCalendar() {
    ...
    selectedDate() {
      ...
      this.$emit("selectCalendar", this.curDate, this.swiperDate[clickedIndex]);
    },
    getSingeDate({day,libsDay}) {
      ...
      this.getDaysInfo(5, initDate, libsDay);
    },
    /**
     * 获取n天的日期数据
     */
    getDaysInfo(n, date, libsDay) {
      let nowTimes = typeof date !== "undefined" ? date.getTime() : new Date().getTime();
      let dateArray = [];
      for (let i = 0; i < n; i++) {
        ...
        if (typeof libsDay !== 'undefined' && libsDay instanceof Array && libsDay.length) {
          buildArray.salePrice = libsDay[i].salePrice
          buildArray.stockModel = libsDay[i].stockModel
          buildArray.stock = libsDay[i].stock 
        }
        dateArray.push(buildArray);
      }
      this.swiperDate = dateArray;
      this.$emit("selectCalendar", this.curDate, this.swiperDate[this.swiperActiveIndex]);
    },
    getCurDate() {
      let date = new Date();
      let seperator1 = "-";
