  ...
  <div class="list-warper-mdse">
    ...
    <mescroll-vue ref="mescroll"
      ...
      <!-- <div class="good-list">
        ...
        >
          <img class="fl good-img" :src="item.linkMobileImg" alt>
      ...
      <div class="mdse-list clearfix">
        <div @click="modeDetail(item)"
             v-for="(item,index) in dataList"
             :key="index"
             class="mdse-item">
          ...
          <h3 class="mdse-name">{{item.nickName}}</h3>
          <div class="mdse-p-info">
            <div class="mdse-p">
              <span class="unit">￥</span>
              <span class="price">{{item.priceShow}}</span>
              <span class="text">起</span>
            </div>
            <span v-if="(item.virtualSale)!=0"
                  class="buy-num">{{item.virtualSale}}人购买</span>
          </div>
        </div>
      </div>
    </mescroll-vue>
...
export default {
  ...
  created () {
    document.title = "商品列表";
    // let url = 'http://testwap.lotsmall.cn/detail/ticket/1249/park2018112119355207922?m_id=78'
    // let reg = /http:\/\/testwap.lotsmall.cn\/detail\/ticket\/-?[1-9]\d*\/-?\w*\?m_id=-?[1-9]\d*/
    // reg.test(url)
  ...
  methods: {
    modeDetail (item) {
      this.$router.push("/detail/mdse?id=" + item.id + "&productCode=" + item.productCode);
    },
    search () {
    ...
    upCallback (page, mescroll) {
      this.getAxios(this.api.shop.list.pagelist, {
        currPage: page.num,
        pageSize: page.size,
