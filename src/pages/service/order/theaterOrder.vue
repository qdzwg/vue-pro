<template>
  <div class="theater-order"
       :class="{overflow:canScroll}">
    <div class="theater-list">
      <div><i>演出名称：</i>{{sessionInfo.sessionName}}</div>
      <div><i>演出场所：</i>{{sessionInfo.theaterName}}</div>
      <div><i>演出时间：</i>{{sessionInfo.playDate}}</div>
      <div><i>演出场次：</i>{{sessionInfo.startTime}}</div>
      <div class="theater-seat-Info"><i>票型信息：</i>
        <div><span v-for="(ticket,index) in ticketList"
                :key="index">【{{ticket.name}}】 {{ticket.amount}}张</span></div>
      </div>
      <div class="theater-seat-Info"><i>座位信息：</i>
        <div class=""><span v-for="(seat,index) in seatSelect"
                :key="index">【{{seat.snRow+"排"+seat.seatName+"座"}}】</span></div>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    subForm () {
      ...
      if (this.flag) {
        ...
        this.seatSelect.map(item => {
          seats.push({
            status: ~~item.initSeatStatus,
            fid: item.fid,
            scol: ~~item.sCol,
            srow: ~~item.sRow,
        ...
        let formData = {
          showModelCode: this.sessionInfo.showModelCode,
          showCode: this.sessionInfo.showCode,
          sessionCode: this.sessionInfo.sessionCode,
          playDate: this.sessionInfo.palyDate,
          startTime: this.sessionInfo.startTime,
          areaCode: this.canSale.areaCode,
          areaName: this.canSale.areaName,
          buyerName: this.buyerName,
          buyerMobile: this.buyerMobile,
          buyerIdNo: this.buyerIdNo,
          wayType: '1',
          idCard: '',
          ttList: JSON.stringify(this.ticketList),
          seats: JSON.stringify(seats),
          orderType: 'theater_ticket',
          leaguerId: localStorage.getItem(
            "xjsc_login_userId_2019_3_8"
          ), // 会员id
          theatreCode: this.sessionInfo.theaterCode,
          price: this.price.toFixed(2),
          amount: this.seatSelect.length,
        };
        this.postAxios("/order/api/lockTtStock", formData).then(
          result => {
    ...
    saveOrder () {
      ...
      let formData = {
        wayType: 1,
        realNames: '',
        modelCodes: this.sessionInfo.showCode,
        areaCode: this.canSale.areaCode,
        amounts: this.seatSelect.length,
      ...
      let env = isWxAli();
      this.postAxios("/order/api/saveOrder", formData).then(
        result => {
          if (result.status != "200") {
            this.$vux.toast.warn(result.message, "middle");
          } else {
    ...
    getCoupons () {
      ...
      let cpArr = [],
        modelprice = "";
      modelprice = this.ticket.code + "-" + this.ticket.price * this.seatSelect.length;
  ...
  created () {
    ...
    this.ticket = JSON.parse(localStorage.getItem("theaterTicket"));
    ...
    this.ticket.map(item => {
      ...
      this.seatSelect.map(seat => {
        if (obj.ttId == seat.ticketId) {
          obj.amount++;
        }
      })
      this.price += obj.price * obj.amount;
