<template>
  <div class="repast-order" :class="{overflow:canScroll}">
        ...
        <div class="hotel-item-main">
          <div class="hotel-info">
            <div class="counpon-title">
              <div class="title">
                <span>{{conponList.conversionName}}</span>
              </div>
              <div class="buy-price clearfix">
                <span class="price fr">{{conponList.shopPrice || conponList.sellingPrice}} 元</span>
              </div>
            </div>
            <div class="numbox clearfix">
              ...
              <span class="addnum fr">
                <div class="px-right">
                  ...
                  <span class="sub" :class="{sub0:mealNum==0}" @click.stop="sub">-</span>
    ...
    <div class="buyer-info">
      <div class="info-head">
        ...
        <img
          src="http://statics.lotsmall.cn/wx/img/icon-contact.png"
    ...
    <div class="bottom-box">
      ...
      <div v-if="mealNum<=0" class="right-warper btn-sub">
        提交订单
      </div> 
      <div v-else class="right-warper" @click="subForm">提交订单</div>      
    </div>
    <!-- 常用联系人 -->
    ...
    <!-- 支付宝支付确认 -->
    <pay ref="pay" :totalprice="totalprice" :isWxPay="isWxPay"></pay>
     <float-nav></float-nav>
  </div>
</template>
...
export default {
  ...
  methods: {
    sub() {      
      if (this.mealNum > 0) {
        this.mealNum--;
        let danjia = this.conponList.shopPrice ? this.conponList.shopPrice : this.conponList.sellingPrice;
        this.subtotal = (danjia * this.mealNum).toFixed(2);      
    ...
    getPrice(obj) {
      ...
      if (obj == 0) {
        ...
        this.totalprice = 0;
        if (this.mealNum > 0) {
          // this.totalprice = _this.conponList.sellingPrice * this.mealNum;
          let danjia = this.conponList.shopPrice ? this.conponList.shopPrice : this.conponList.sellingPrice;
          this.totalprice = danjia * this.mealNum;
        }
    ...
    subForm() {
      ...
      if (this.flag) {
        ...
        formData.modelCode = this.conponList.conversionCode;
        formData.amounts = this.mealNum;
        formData.conversionInfoDto = JSON.stringify(this.conponList);
        formData.orderInfo = this.conponList.conversionName;
        formData.merchantUserInfoId = this.conponList.merchantUserInfoId;
        formData.userInfoId = localStorage.getItem("xjsc_login_userId_2019_3_8") || this.$cookie.get("leaguerInfoId");
        let _url = '/rmsrepast/order/saveOrder';
        let env = isWxAli();
